...
window.inicializarJuegoDamas = function(chatId, miNombre, otroNombre) {
  ...
  listenerDamasActivo = onValue(juegoRef, (snapshot) => {
    ...
    console.log("Estado del juego cargado:", estadoJuego);
    
    if (!estadoJuego) {
      console.log("No hay juego, creando tablero vacío 7x7");
      estadoJuego = obtenerTableroInicial(miNombre, otroNombre);
      update(juegoRef, estadoJuego);
      return;
    }
    
    // Verificar si es un tablero de damas antiguo (tiene propiedades como jugadorBlanco/jugadorRojo)
    // o si el tablero no es 7x7
    if (estadoJuego.jugadorBlanco || estadoJuego.jugadorRojo || 
        !estadoJuego.fichas || estadoJuego.fichas.length !== 7 ||
        (estadoJuego.fichas[0] && estadoJuego.fichas[0].length !== 7)) {
      console.log("⚠️ Detectados datos antiguos de damas o tablero corrupto. Reiniciando...");
      estadoJuego = obtenerTableroInicial(miNombre, otroNombre);
      update(juegoRef, estadoJuego);
      return;
    }
    
    // Verificar si el tablero ya tiene fichas cuando debería estar vacío (datos corruptos)
    let tieneFichas = false;
    if (estadoJuego.fichas) {
      for (let r = 0; r < 7; r++) {
        for (let c = 0; c < 7; c++) {
          if (estadoJuego.fichas[r][c] === 'R' || estadoJuego.fichas[r][c] === 'N') {
            tieneFichas = true;
            break;
          }
        }
        if (tieneFichas) break;
      }
    }
    
    // Si el juego parece ser uno nuevo (no hay ganador) pero tiene fichas, es probablemente corrupto
    if (tieneFichas && !estadoJuego.ganador) {
      console.log("⚠️ Tablero con fichas pero sin ganador - posible corrupción. Reiniciando...");
      estadoJuego = obtenerTableroInicial(miNombre, otroNombre);
      update(juegoRef, estadoJuego);
      return;
    }

...
window.inicializarJuegoDamas = function(chatId, miNombre, otroNombre) {
  ...
  listenerDamasActivo = onValue(juegoRef, (snapshot) => {
    ...
    const esDamasAntiguo = estadoJuego.jugadorBlanco || estadoJuego.jugadorRojo;
    const tableroCorrupto = !estadoJuego.fichas || estadoJuego.fichas.length !== 7 ||
                          (estadoJuego.fichas[0] && estadoJuego.fichas[0].length !== 7);
    
    if (esDamasAntiguo || tableroCorrupto) {
      console.log("⚠️ Detectados datos antiguos de damas o tablero corrupto. ELIMINANDO y recreando...");
      // Eliminar completamente los datos antiguos
      update(juegoRef, null).then(() => {
        // Esperar un momento y crear nuevo tablero
        setTimeout(() => {
          const nuevoTablero = obtenerTableroInicial(miNombre, otroNombre);
          update(juegoRef, nuevoTablero);
        }, 100);
      });
      return;
    }
...
function renderizarTableroCincoEnRaya(estado, chatId, miNombre) {
  console.log("renderizarTableroCincoEnRaya llamado con estado:", estado);
  
  const grid = document.getElementById("damas-grid");
  const turnInfo = document.getElementById("damas-turn-info");
  
  if (!grid) {
    console.error("No se encontró el elemento damas-grid");
    return;
  }
  if (!turnInfo) {
    console.error("No se encontró el elemento damas-turn-info");
    return;
  }
